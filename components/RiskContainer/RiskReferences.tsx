import React from "react";

const RiskReferences: React.FC = () => {
  return (
    <div className="prose prose-neutral max-w-none dark:prose-invert">
      <ul>
        <li>
          <strong>Initial Derivation/Validation:</strong>
          <ul>
            <li>
              Li A, et al. Derivation and Validation of a Clinical Risk Assessment
              Model for Cancer-Associated Thrombosis in Two Unique US Health Care
              Systems. <em>J Clin Oncol</em>. 2023;41(16):2926-2938. doi:{" "}
              <a
                href="https://doi.org/10.1200/JCO.22.01542"
                target="_blank"
                rel="noopener"
              >
                10.1200/JCO.22.01542
              </a>
              . PMID:{" "}
              <a
                href="https://pubmed.ncbi.nlm.nih.gov/36626707/"
                target="_blank"
                rel="noopener"
              >
                36626707
              </a>
              .
            </li>
          </ul>
        </li>
        <li>
          <strong>External Validation:</strong>
          <ul>
            <li>
              Li A, et al. External validation of a novel electronic risk score for
              cancer-associated thrombosis in a comprehensive cancer center.{" "}
              <em>Am J Hematol</em>. 2023;98(7):1052-1057. doi:{" "}
              <a
                href="https://doi.org/10.1002/ajh.26928"
                target="_blank"
                rel="noopener"
              >
                10.1002/ajh.26928
              </a>
              . PMID:{" "}
              <a
                href="https://pubmed.ncbi.nlm.nih.gov/37067102/"
                target="_blank"
                rel="noopener"
              >
                37067102
              </a>
              .
            </li>
            <li>
              Dulberger KN, et al. External validation of a novel cancer-associated venous 
              thromboembolism risk assessment score in a safety-net hospital.{" "}
              <em>Res Pract Thromb Haemost</em>. 2024;9(1):102650. doi:{" "}
              <a
                href="https://doi.org/10.1016/j.rpth.2024.102650"
                target="_blank"
                rel="noopener"
              >
                10.1016/j.rpth.2024.102650
              </a>
              . PMID:{" "}
              <a
                href="https://pubmed.ncbi.nlm.nih.gov/39839661/"
                target="_blank"
                rel="noopener"
              >
                39839661
              </a>
              .
            </li>
          </ul>
        </li>
        <li>
          <strong>Additional Reference:</strong>
          <ul>
            <li>
              Li A, et al. Developing and optimizing a computable phenotype for
              incident venous thromboembolism in a longitudinal cohort of patients
              with cancer. <em>Res Pract Thromb Haemost</em>. 2022;6(4):e12733. doi:{" "}
              <a
                href="https://doi.org/10.1002/rth2.12733"
                target="_blank"
                rel="noopener"
              >
                10.1002/rth2.12733
              </a>
              . PMID:{" "}
              <a
                href="https://pubmed.ncbi.nlm.nih.gov/35647478/"
                target="_blank"
                rel="noopener"
              >
                35647478
              </a>
              .
            </li>
          </ul>
        </li>
      </ul>
    </div>
  );
};

export default RiskReferences;
